...
export const tfsaCurrentBalance = createSelector(
    [netWorthAssets],
    (netWorthAssets) =>  Object.values(netWorthAssets).length > 0 ? Object.values(netWorthAssets).filter(d => d.registration === "tfsa").map(d => d.currentValue.financialValue).reduce((acc, num) => acc + num) : 0
)
...
export const tfsaProjection_selector = createSelector(                                                                      //Determines the CPP payment for the user
    ...
    rate2,
    tfsaCurrentBalance,
    (savings_reducer, userAge, lifeSpan, rate1, rate2, tfsaCurrentBalance) => createProjection(savings_reducer, userAge, lifeSpan, rate1, rate2, tfsaCurrentBalance)                                       
)
