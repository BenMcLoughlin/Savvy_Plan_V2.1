...
import {changeLabel_action, changeIncome_action, delete_action, changeAge_action} from "redux/income/income_actions"
import _ from "lodash"
import {incomeStream_data} from "pages/income/data/income_data"
import {cpp_selector} from "redux/income/income_selectors"


const EditIncome = ({category, instanceArray, changeIncome_action, createNewItem, id, setId, changeLabel_action, changeAge_action, setCategory, delete_action}) => {    

    const setValue = (logValue, rangeBarValue, rangeBarProps) => {                                                             //receives numbers from range bar and sets them in state
        changeIncome_action(id, logValue, rangeBarValue, rangeBarProps)                                                        //setting the income value in the reducer
    }
    ...
    const deleteInstance = (instance) => {                                                                                     //deletes the instance
        if (instance.id === id) {                                                                                              //checks if the instance being deleted and the one currently being displayed are the same
            if (instanceArray.length > 0) {                                                                                    // if the array is greater then one it wil delete the instance and change the id of the instance being displayed
                ...
                delete_action(instance.id)                                                                                     //removes the instance
            }
...
export default connect(mapStateToProps, {changeLabel_action, changeIncome_action, delete_action, changeAge_action})(EditIncome )
