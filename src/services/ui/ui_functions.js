...
export const editStreamName = (e, income_selector, stream, setNestedKeyValue_action, setKeyValue_reducer, reducer) => {                                                                                               //enables the user to change the label which changes all income stream labels of that stream
    const {value} = e.target                                                                                               //destructure out the value from the target event
   const instanceArray =  Object.values(income_selector).filter(d => d.stream === stream)
    ...
    for (let i = 0; i < instanceArray.length; i++) {                                                                       //we loop through and change the label for every income stream in the stream
       ...
       setKeyValue_reducer("stream", "ui_reducer", e.target.value)                                                                                        //sets the stream
}
...
export const setAge = (id, instanceArray, name, setNestedKeyValue_action, reducer, value) => {                             //sets the age, as well as the surrounding ages in the array of instances
    ...
    //const stream = income_selector[id].stream
    //const instanceArray =  Object.values(income_selector).filter(d => d.stream === stream).sort((a,b) => a.age1 - b.age1)   
...
export const hideStream = (setKeyValue_action) => {
