...
export const calcIncome = (age, reducer) =>  {
    return (Object.values(reducer).map(d => age >= d.age1 && age <= d.age2 ? d.value : 0 ).reduce((acc, num) => acc + num))}

export const calcNonTaxableIncome = (age, reducer) =>  Object.values(reducer).map(d => d.reg === "TFSA" && age >= d.age1 && age <= d.age2 ? d.value : 0 ).reduce((acc, num) => acc + num)
export const calcCPP = (inc) =>  inc 
export const calcTax = (age, reducer, query) =>  reducer.toArray().map(d => d.type === query && d.eligible && age >= d.age1 && age <= d.age2 ? d.value : 0 ).sum()
...
export const bracketsToChartData = (bracket) => bracket.map((d,i,a) => ({
                                                                bracketIncome: d.inc,
...
export const calculateTaxes = (age, income_selector, tax_selector) => {
    ...
    let afterTaxIncome = postDedIncome - postDedFedTaxes -postDedProvTaxes + totalCredSavings
...
const array = []
     for (let age = age1; age<= age2; age++) {
       const {postDedFedTaxes, credFedTaxSavings,postDedProvTaxes, credProvTaxSavings } = calculateTaxes(age, income_reducer, tax_reducer)

             array.push({
                 age:  age, 
                 federalTax: postDedFedTaxes -  credFedTaxSavings,
                 provincialTax: postDedProvTaxes - credProvTaxSavings,
             })
     }
     return array
}   
